...
let fc = document.getElementById("ppFc")
const modsImgs = {
    'nm': './static/nomod.png',
    'ez': './static/easy.png',
...
socket.onmessage = event => {
	...
	if (data.gameplay.pp.current != '') {
		let ppData = Math.round(data.gameplay.pp.current);
        let ifFC = Math.round(data.gameplay.pp.fc);
		// pp.innerHTML = ppData.toString() + "/" + ifFC.toString()
        pp.innerHTML = ppData
	} else {
		pp.innerHTML = "";
	}
    if (data.gameplay.pp.fc != '') {
		let ppData = Math.round(data.gameplay.pp.fc);
		// pp.innerHTML = ppData.toString() + "/" + ifFC.toString()
        fc.innerHTML = ppData
	} else {
		fc.innerHTML = "";
